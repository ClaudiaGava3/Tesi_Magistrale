...
def plot_brs(params, model, controller, nn_model, mean, std, x_data_raw, grid=1.2e-2):
    """ Plot the Viability Kernel boundary for a fixed maximum room scale. """
    print ('Plotting Viability Kernel Boundary...')
    import matplotlib.pyplot as plt
    from matplotlib.ticker import AutoMinorLocator
    
    # --- 1. DEFINIAMO LA STANZA VIRTUALE (Il limite del Kernel) ---
    # Scegliamo un fattore di scala massimo tollerabile. Es: 1.5 
    MAX_ROOM_SCALE = 0.55

    with torch.no_grad():
        # 2. Griglia delle velocità
        vx_grid = np.arange(-1.0, 1.0 + grid, grid)
        vz_grid = np.arange(-1.0, 1.0 + grid, grid)
        vx, vz = np.meshgrid(vx_grid, vz_grid)
        vx_rav, vz_rav = vx.ravel(), vz.ravel()
        n = len(vx_rav)
        
        # 3. Input per la rete: [theta, vx, vz, wy] con drone dritto (theta=0, wy=0)
        x_in = np.zeros((n, 4))
        x_in[:, 1] = vx_rav
        x_in[:, 2] = vz_rav
        ...
        # 4. Predizione della rete (l'output ora è 1 singolo numero: lo scaling)
        device = next(nn_model.parameters()).device
        tensor_in = torch.from_numpy(x_in_std.astype(np.float32)).to(device)
        y_pred = nn_model(tensor_in).cpu().numpy()
        
        # 5. CREIAMO IL CONFINE: 1 se lo scaling <= MAX_ROOM_SCALE, 0 altrimenti
        viable = (y_pred[:, 0] <= MAX_ROOM_SCALE).astype(int)
        z = viable.reshape(vx.shape)
        
        # 6. Plot in perfetto stile originale
        plt.figure(figsize=(8, 6))
        plt.rcParams.update({"text.usetex": False, "font.family": "serif"})

        # Disegna il confine netto (Blu = Viable, Rosso/Bianco = Not Viable)
        # Forziamo Matplotlib a capire che i valori vanno da 0 a 1
        plt.contourf(vx, vz, z, levels=[-0.5, 0.5, 1.5], cmap='coolwarm_r', alpha=0.8)

        # OVERLAY DEI PUNTI SIMULATI (Indici in x_data_raw: 0:theta, 1:vx, 2:vz, 3:wy)
        plt.scatter(x_data_raw[:, 1], x_data_raw[:, 2], color='darkblue', s=4, label='Sampled Data', zorder=5)
        ...
        plt.title(f'Viability Kernel for Scale $\leq$ {MAX_ROOM_SCALE}', fontsize=14)
        ...
        plt.savefig(params.PLOTS_DIR + '/brs/' + f'viability_kernel_border.pdf', format='pdf', bbox_inches='tight')
        plt.show(block=False)