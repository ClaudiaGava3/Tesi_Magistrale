...
def plot_brs(params, model, controller, nn_model, mean, std, x_data_raw, grid=1.2e-2):
    ...
    print ('Plotting Viability Kernel Boundary 3D (Slice vx vs vz)...')
    import matplotlib.pyplot as plt
    from matplotlib.ticker import AutoMinorLocator
    ...
    with torch.no_grad():
        ...
        # In 3D abbiamo 9 input: [phi, theta, psi, vx, vy, vz, p, q, r]
        # Manteniamo angoli a 0, vy a 0, e omega a 0. Cambiamo solo vx (idx 3) e vz (idx 5)
        x_in = np.zeros((n, 9))
        x_in[:, 3] = vx_rav  # vx
        x_in[:, 5] = vz_rav  # vz
        ...
        # OVERLAY: gli indici in x_data_raw (che conteneva solo le 9 var) per vx e vz sono 3 e 5
        plt.scatter(x_data_raw[:, 3], x_data_raw[:, 5], color='darkblue', s=4, label='Sampled Data', zorder=5)
        ...
        plt.title(f'Viability Kernel Slice (Scale $\leq$ {MAX_ROOM_SCALE})', fontsize=14)
        ...
        plt.savefig(params.PLOTS_DIR + '/brs/viability_kernel_border_3D.pdf', format='pdf', bbox_inches='tight')
        plt.show(block=False)