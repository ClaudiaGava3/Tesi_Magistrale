...
def plot_histogram(
    ...
    bins: int = 30,
    saving_dir: str = "plots/histograms_cube/",
    xticks: list = None,
...
if __name__ == '__main__':
    ...
    nn_filename = f'{params.NN_DIR}{robotic_system}_{params.act}_randB_cube.pt'
    ub = 1
    ...
    if params.generation:
        ...
        b_init = np.ones((params.prob_num, model.nbox))
        ...
        for nb in range(n_batch):  
            ...
            np.save(f'{params.DATA_DIR}{robotic_system}_x_vboc_randB_cube', x_data)
            np.save(f'{params.DATA_DIR}{robotic_system}_b_vboc_randB_cube', b_optimized)
            np.save(f'{params.DATA_DIR}{robotic_system}_n_horizons_vboc_randB_cube', n_data)
            np.save(f'{params.DATA_DIR}{robotic_system}_status_vboc_randB_cube', status_list)
            # === SALVATAGGIO DEI FALLIMENTI ===
            np.save(f'{params.DATA_DIR}{robotic_system}_failed_q_init_vboc_randB_cube', np.array(all_failed_q_init))

            np.save(f'{params.DATA_DIR}{robotic_system}_actual_boxes_vboc_randB_cube', actual_boxes)
            np.save(f'{params.DATA_DIR}{robotic_system}_traj_kinematics_vboc_randB_cube', traj_kinematics)
            np.save(f'{params.DATA_DIR}{robotic_system}_u_traj_vboc_randB_cube', np.vstack(u_traj))

            np.save(f'{params.DATA_DIR}{robotic_system}_TEST_dataset_classification_cube', np.array(all_test_dataset))
...
# =================================================
    ...
    if params.training: 
        ...
        # --- Load data ---
        x_data = np.load(f'{params.DATA_DIR}{robotic_system}_x_vboc_randB_cube.npy')
        b_data = np.load(f'{params.DATA_DIR}{robotic_system}_b_vboc_randB_cube.npy')
        ...
        if params.plot:
            hist_dir = os.path.join(plots_dir, 'histograms_cube')
            ...
            failed_file = f'{params.DATA_DIR}{robotic_system}_failed_q_init_vboc_randB_cube.npy'
        ...
        train_val_dir = os.path.join(plots_dir, 'training_validation_cube')
        ...
        loss_dir = os.path.join(plots_dir, 'loss_evolution_cube')
