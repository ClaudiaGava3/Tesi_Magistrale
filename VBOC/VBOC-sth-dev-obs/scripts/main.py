...
def compute_data_on_border(
    ...
    x_guess[:, :6] = q_init      # Posizioni e velocità (6 elementi)
    x_guess[:, 6] = box_guess       # Guess iniziale per il parametro delle dimensioni del box
    ...
    x_flat = np.hstack((q_init[:2], [0.0], q_init[3:6], [box_guess]))
    allocation_matrix = model.R(x_flat).full() @ model.F

    u_hover = np.linalg.pinv(allocation_matrix) @ np.array([0, 0, model.mass * model.g])
...
if __name__ == '__main__':
    ...
    if params.generation:
        ...
        for nb in range(n_batch):  
            ...
            b_optimized = x_data[:, 6].reshape(-1, 1)
...
# =========================================================================
    ...
    if params.training: 
        ...
        # --- Histograms of raw data distributions 2D ---
        if params.plot:
            hist_dir = os.path.join(plots_dir, 'histograms')
            ensure_clean_dir(hist_dir)
            ...
            plot_histogram(
                x_data[:, 2:6], 
                title="Inputs_Theta_Vx_Vz_Wy", 
                xlabel="Value",
                ylabel="Frequency", 
                bins=50, 
                saving_dir=hist_dir
            )
            # 2. Istogramma dei Target: le 4 dimensioni del Box ottimizzato
            plot_histogram(
                b_data, 
                title="Targets_Box_Dimensions", 
                xlabel="Value [m]", 
                ylabel="Frequency", 
                bins=50, 
                saving_dir=hist_dir
            )
            # 3. Istogramma dello Status del Solver
            plot_histogram(
                status_data, 
                title="Solver_Status", 
                xlabel="Status Code", 
